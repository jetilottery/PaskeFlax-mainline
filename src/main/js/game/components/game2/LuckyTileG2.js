define((require) => {
    ...
    class LuckyTileG2 extends ScratchSymbol {
        constructor(number) {
            super(60, 75, {
                background: "eggTile_O",
                win: "none",
                lose: "none",
                foil: "eggFoil" + number + "G2"
            });
        ...
        hideBG() {
            this.background.texture = PIXI.Texture.fromFrame("eggTile_Blank");
        }

        showBG() {
            if(this.letterName && this.letterName != "") {
                this.background.texture = PIXI.Texture.fromFrame("eggTile_" + this.letterName);
            } else {
                this.background.texture = PIXI.Texture.fromFrame("eggTile_Blank");
            }
        }

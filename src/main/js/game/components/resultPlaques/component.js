define(function(require) {
    ...
    return function resultPlaques(parts) {
        ...
        //Replace win title with curved text
        let curvedCongratulationsRP = curveText(parts.congratulationsRP, 198, -1);
        parts.winPopUp.removeChild(parts.congratulationsRP);
        parts.winPopUp.addChild(curvedCongratulationsRP);

        function waitRandom() {
            let wait = Math.floor(Math.random() * 5) + 8;
            Tween.delayedCall(wait, blink);
        }

        function blink() {
            if(parts.cheekyChickenWinPlaque.visible) {
                parts.cheekyChickenWinPlaque.gotoAndPlay(0);
            }
            waitRandom();
        }
        ...
        function showPlaque() {
            ...
            if(meterData.totalWin === 0) {
                ...
                parts.winPlaque.visible = false;
                parts.cheekyChickenWinPlaque.visible = false;
                ...
                parts.cheekyChickenWinPlaque.visible = true;
                parts.resultPlaques.visible = true;
